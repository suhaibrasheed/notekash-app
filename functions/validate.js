...
// onRequestPost automatically handles only POST requests
export async function onRequestPost(context) {
    try {
        // context.request holds the incoming request, context.env holds your secrets
        ...
        if (!JWT_SECRET) {
            return new Response(JSON.stringify({ error: 'Server error: Missing JWT_SECRET.' }), { status: 500 });
        }
        ...
        if (!foundTier) {
            return new Response(JSON.stringify({ error: 'Invalid license key.' }), { status: 403 });
        }
        ...
        // Sign token using the Cloudflare-specific JWT library
